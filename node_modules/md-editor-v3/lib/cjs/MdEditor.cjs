...
const scrollAuto = (pEle, cEle, codeMirrorUt) => {
  ...
  const pEleHandler = () => {
    ...
    scale = (scrollDOM.scrollTop - startTop + getComputedStyleNum(view2.contentDOM, "padding-bottom")) / (endBottom - startTop);
    if (startTop > 0 && (endBottom >= pMaxScrollLength || endEle.offsetTop + endEle.clientHeight > cMaxScrollLength)) {
      ...
      startEleOffetTop = (_c = document.querySelector(`[data-line="${lineNumer}"]`)) == null ? void 0 : _c.offsetTop;
  ...
  const cEleHandler = () => {
    ...
    if (endLineScrollTop > pMaxScrollLength || realEleEnd.offsetTop + realEleEnd.offsetHeight > cMaxScrollLength) {
      const lineNumer = getLineNumber(pMaxScrollLength, cMaxScrollLength);
      eleStartOffsetTop = (_f = document.querySelector(
        `[data-line="${lineNumer}"]`
      )) == null ? void 0 : _f.offsetTop;
